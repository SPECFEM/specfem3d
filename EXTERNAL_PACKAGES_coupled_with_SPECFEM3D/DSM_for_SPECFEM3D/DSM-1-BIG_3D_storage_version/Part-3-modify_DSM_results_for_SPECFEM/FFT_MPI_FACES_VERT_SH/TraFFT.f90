...
!-----------------------------------------------------------------------
  ...
  integer :: itranslat
  character(250) :: coutfile,coutfile1
...
!! VM VM supress one file and change read statement
     ...
     if (iPS /= 2) then ! SH calculation  ! TO DO changer l'ordre ista,irpof dans tmpsngl(1)
        ...
        open(1,file=coutfile,status='unknown',form='unformatted', &
             access = 'direct', recl=2*6*6*kind(0e0)*nsta*r_n_global)
        read(1,rec=k) tmpsngl(1:6,1:6,1:nsta,1:r_n_global)
        stresssngl(1:6,1:6,1:nsta,1:r_n,i)=stresssngl(1:6,1:6,1:nsta,1:r_n,i)+tmpsngl(1:6,1:6,1:nsta,iprofmin:iprofmax)
        close(1)
        ...
        open(1,file=coutfile,status='unknown',form='unformatted', &
             access = 'direct', recl=2*3*6*kind(0e0)*nsta*r_n_global)
        read (1,rec=k) tmpsngl1(1:3,1:6,1:nsta,1:r_n_global)

        displacementsngl(1:3,1:6,1:nsta,1:r_n,i)=displacementsngl(1:3,1:6,1:nsta,1:r_n,i)+tmpsngl1(1:3,1:6,1:nsta,iprofmin:iprofmax)
...
!!  open(111,file=coutfile1, status = 'old', form='formatted')
!   write(111,*) 6371.d0-r_(iprof),stla(i),stlo(i),updown(iprof)
!!   write(111,*) stlo(i),stla(i),(6371.d0-r_(iprof))*1d3
!   write(111,'("# ",i5.4,i4.4)') i,order+myrank*4
!!  close(111)
...
!    if (iprofmin == 11) then ! just for the test.
!       write(*,*) 'iprofmin',iprofmin,iprofmax
!       write(*,*) 'radiu is',i,r_(1)
