program ChangeFormat
   ...
   character(120) :: coutfile,coutfile1,outputDir,psvmodel,modelname,stationsinf,parentDir
   ...
   integer ibloc,ns,irec,imin,imax,order,j,jj
   integer k1,k2,k3,k4
   integer norder,irek
...
!      do j = 0,11999
!            write(111,*) dble(j)/1.0d1,SeiLoc(j+1 ,jj,ista,ir)
!       enddo
!          close(111)
!    enddo
!     order = order + 1
...
!endif
   ...
   nbloc = int(nt/nbrc)
   if (myrank == 0) then
     open(20,file=trim(Outputdir)//'/Disp.bin',form='unformatted')
