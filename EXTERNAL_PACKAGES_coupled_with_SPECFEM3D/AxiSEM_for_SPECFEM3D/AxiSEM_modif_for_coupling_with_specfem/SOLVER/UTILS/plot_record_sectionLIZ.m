...
for j=1:3
  ...
  figure(j); for i=1:nrec
       plot(seis(:,i,j)/globmax(1)*epidist(i)^1.5/(epidist(1)+100)*deltadist/nrec+epidist(i),time,'k')

      % plot station names
      textfile=fgets(fid2,100); filen2=sprintf(strtrim(textfile)); text(time(1),epidist(i),sprintf('%3s',filen2))
   end
