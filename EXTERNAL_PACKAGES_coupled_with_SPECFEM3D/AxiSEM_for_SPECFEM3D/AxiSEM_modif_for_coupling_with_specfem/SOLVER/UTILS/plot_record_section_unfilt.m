...
if tau_exist > 0
load('TAUP/taup_PcP_traveltime.dat'); load('TAUP/taup_S_traveltime.dat')
...
if max(epidist) >95. ;load('TAUP/taup_Sdiff_traveltime.dat'); load('TAUP/taup_Pdiff_traveltime.dat'); end

for j=1:3
   figure(j)
   plot(taup_P_traveltime(:,2),taup_P_traveltime(:,1),'r');  hold on
   plot(taup_PP_traveltime(:,2),taup_PP_traveltime(:,1),'r--')
