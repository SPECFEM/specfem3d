...
! reads in smoothed kernels from former iteration in OUTPUT_SUM.old/ : bulk, betav, betah, eta
  ...
  allocate(kernel_betav_old(NGLLX,NGLLY,NGLLZ,NSPEC))
  allocate(kernel_betah_old(NGLLX,NGLLY,NGLLZ,NSPEC))
  ...
  if (USE_OLD_GRADIENT) then
    ...
    allocate(model_dbetav_old(NGLLX,NGLLY,NGLLZ,NSPEC))
    allocate(model_dbetah_old(NGLLX,NGLLY,NGLLZ,NSPEC))
