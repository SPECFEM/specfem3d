...
! open parameter file Mesh_Par_file
  ...
  allocate(material_properties(NMATERIALS,7),stat=ierr)
  if (ierr /= 0) stop 'Error allocation of material_properties'
  material_properties(:,:) = 0.d0
  do imat = 1,NMATERIALS
    call read_material_parameters(IIN,mat_id,rho,vp,vs,Q_flag,anisotropy_flag,domain_id,ierr)
    if (ierr /= 0) stop 'Error reading materials in Mesh_Par_file'
    ! stores material
  ...
  do ireg =1,NSUBREGIONS
    ...
    if (ierr /= 0) stop 'Error reading regions in Mesh_Par_file'
    ! stores region
