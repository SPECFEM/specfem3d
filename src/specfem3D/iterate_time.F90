  ...
  if (SIMULATION_TYPE == 1 .and. SAVE_FORWARD) then
    ...
    if (APPROXIMATE_HESS_KL) then
      if (ELASTIC_SIMULATION) &
           call transfer_kernels_hess_el_tohost(Mesh_pointer,hess_kl,hess_rho_ac_kl,hess_kappa_ac_kl,NSPEC_AB)
      if (ACOUSTIC_SIMULATION) &
           call transfer_kernels_hess_ac_tohost(Mesh_pointer,hess_ac_kl, hess_rho_kl, hess_kappa_kl, hess_mu_kl, NSPEC_AB)
    endif
