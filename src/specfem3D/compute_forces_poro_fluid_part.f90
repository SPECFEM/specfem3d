...
!---
  ...
  integer :: ispec,ispec_irreg,i,j,k,l,iglob,num_elements,ispec_p
      ...
      do j = 1,NGLLY
        do i = 1,NGLLX
          ...
          if (etal_f > 0.0_CUSTOM_REAL) then
            ...
            accelw_poroelastic(2,iglob) = accelw_poroelastic(2,iglob) &
                                        - wxgll(i)*wygll(j)*wzgll(k)*jacobianl*viscodampy
            accelw_poroelastic(3,iglob) = accelw_poroelastic(3,iglob) &
                                        - wxgll(i)*wygll(j)*wzgll(k)*jacobianl*viscodampz
