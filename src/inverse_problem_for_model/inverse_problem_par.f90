...
module inverse_problem_par
  ...
  !!! verbose mode with outputs for checking the FWI
  ...
  !! big number of stations (not used yet)
  ...
  ! inversion iterators
  integer,                       public             :: iter_inverse, iter_frq, iter_wolfe
...
!################################################# STRUCTURES ###############################################
  ...
  type, public :: inver
     ...
     real(kind=CUSTOM_REAL)                                                   :: current_step_length = 1.0

     !! weight for regularization on family parameters
     real(kind=CUSTOM_REAL), dimension(:), allocatable                        :: reg_family, damp_family

     !! max perturbation allowed for initial guess step length relative to gradient max value
     real(kind=CUSTOM_REAL)                                                   :: max_relative_pert = 0.05 ! (5%)
     real(kind=CUSTOM_REAL), dimension(:), allocatable                        :: stats_max_relative_pert
     ...
     logical                                                                  :: dump_image_at_each_iteration = .true.
     !! user-defined taper
     ...
     real(kind=CUSTOM_REAL)                                                   :: zPrc1 = -200., zPrc2 = -400., aPrc = 3.0
     ...
     !!  cost function for each event (not used yet)
     ...
     real(kind=CUSTOM_REAL)                                                   :: data_std = 0.0, nb_data_std = 0.0

     real(kind=CUSTOM_REAL)                                                   :: prior_data_std = 1.0
     ...
     !! regularization ----------------------------------------------------------------------------------------------------
     logical                                                                  :: use_regularization_FD_Tikhonov = .false.
     logical                                                                  :: use_regularization_SEM_Tikhonov = .false.
     logical                                                                  :: use_damping_SEM_Tikhonov = .false.
     logical                                                                  :: use_variable_SEM_damping_sources = .false.
     logical                                                                  :: use_variable_SEM_damping_stations = .false.
     real(kind=CUSTOM_REAL)                                                   :: weight_Tikhonov = 0.1
     real(kind=CUSTOM_REAL)                                                   :: weight_Tikhonov_normalized
     real(kind=CUSTOM_REAL)                                                   :: cost_penalty
     ! spatial damping sources
     real(kind=CUSTOM_REAL)                                                   :: min_damp_src = 1.0, max_damp_src = 10.0
     real(kind=CUSTOM_REAL)                                                   :: distance_from_source = 100.0
     ! spatial damping stations
     real(kind=CUSTOM_REAL)                                                   :: min_damp_sta = 1.0, max_damp_sta = 10.0
     real(kind=CUSTOM_REAL)                                                   :: distance_from_station = 100.0
     ...
     character(len=3)                                                         :: inverted_data_sys = 'xyz'
     logical, dimension(3)                                                    :: inverted_data_comp = (/ .true., .true., .true. /)
     character                                                                :: inverted_data_type = 'd'
     logical                                                                  :: is_src_weight_gradient = .false.
     logical                                                                  :: convolution_by_wavelet = .false.
  ...
  type, public :: acqui
     ...
     !! window used for FWI (NCOMP,2,NSTA) - todo not used yet
