...
!-------------------------------------------------------------------------------------------------------------------
  subroutine read_acqui_teleseismic_file(acqui_file, acqui_simu, myrank)
    ...
    if (myrank == 0) then
      ...
      ! debug
      if (DEBUG_MODE) write(IIDD,*) ' opening  ', trim(acqui_file), ' mygroup ', mygroup
      ...
      do
        ...
        case('event_name')
          ...
          if (trim(adjustl(acqui_simu(ievent)%source_wavelet_file)) /= 'undef') then
            ...
            call read_binary_source_signature(acqui_simu(ievent)%source_wavelet_file,nt, &
    ...
    do ievent = 1, NEVENT
       ...
       do ireceiver = 1, NSTA
          ...
          call get_MPI_slice_and_bcast_to_all(x_to_locate, y_to_locate, z_to_locate, x_found, y_found, z_found, &
                                              xi_receiver, eta_receiver, gamma_receiver, ispec_selected, islice_selected, &
                                              distance_from_target, myrank)
