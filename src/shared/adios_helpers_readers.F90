...
!
  ...
  ! ADIOS2: A string variable like undef_mat_prop/array is not a GlobalArray,
  !         it can't have a selection, in call to SetSelection
  !
  ! here we're using a slight modification to generic reader routine.
  ...
  TRACE_ADIOS_L2_ARG('read_adios_schedule_array_global_1d_string_1d: ',trim(array_name))

  ! checks name
  if (len_trim(array_name) == 0) stop 'Error: array_name has zero length in read_adios_schedule_array_global_1d_string_1d'
  ...
  call check_adios_err(ier,"Error adios2 readers_1d_string: inquire variable "//trim(array_name)//" failed")

  if (.not. v%valid) stop 'Error adios2 variable invalid'

  ! no selection for string variables...
  !call adios2_set_selection(v, 1, start, count, ier)
