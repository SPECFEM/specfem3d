...
! option (use a "grep" command to find them) to see how this could/should be done
  ...
  call read_value_logical(CUT_SOLUTION_FOR_VISU, 'CUT_SOLUTION_FOR_VISU', ier)
  if (ier /= 0) stop 'Error reading Par_file parameter CUT_SOLUTION_FOR_VISU'
...
! to avoid an I/O bottleneck in the case of very large runs
  if(BROADCAST_AFTER_READ) then
    ...
    call bcast_all_singlel_world(CUT_SOLUTION_FOR_VISU)
