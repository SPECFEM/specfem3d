...
! slices copy the right face into the buffer
  do ipoin=1,npoin2D_xi
    buffer_send_faces(:,ipoin) = array_val(:,iboolright_xi(ipoin))
  enddo
...
! all slices copy the left face into the buffer
  do ipoin=1,npoin2D_xi
    buffer_send_faces(:,ipoin) = array_val(:,iboolleft_xi(ipoin))
  enddo
...
! all slices copy the buffer received to the contributions on the right face
  if(iproc_xi < NPROC_XI-1) then
  do ipoin=1,npoin2D_xi
    array_val(:,iboolright_xi(ipoin)) = buffer_received_faces(:,ipoin)
  enddo
  endif
...
! slices copy the right face into the buffer
  do ipoin=1,npoin2D_eta
    buffer_send_faces(:,ipoin) = array_val(:,iboolright_eta(ipoin))
  enddo
...
! all slices copy the left face into the buffer
  do ipoin=1,npoin2D_eta
    buffer_send_faces(:,ipoin) = array_val(:,iboolleft_eta(ipoin))
  enddo
...
! all slices copy the buffer received to the contributions on the right face
  if(iproc_eta < NPROC_ETA-1) then
  do ipoin=1,npoin2D_eta
    array_val(:,iboolright_eta(ipoin)) = buffer_received_faces(:,ipoin)
  enddo
  endif
