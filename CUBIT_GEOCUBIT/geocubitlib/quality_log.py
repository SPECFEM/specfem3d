...
def quality_log(tqfile=None):
    ...
    if len(lvol) != 0:
        ...
        cubit.cmd(
            'quality hex in block all allmetric')
    ...
    if mesh.min_edge_length is not None:
        mesh.edgemin_hystogram = mesh.hyst(
            mesh.min_edge_length, mesh.max_edge_length, mesh.edgemin_hyst)
        mesh.edgemax_hystogram = mesh.hyst(
            mesh.min_edge_length, mesh.max_edge_length, mesh.edgemax_hyst)
        totstat_file.write('=' * 70 + '\n')
        totstat_file.write('edge length')
        totstat_file.write('=' * 70 + '\n')
        if len(mesh.hex_min_edge_length) <= 30:
            totstat_file.write('minimum edge length: ' +
                               str(mesh.min_edge_length) +
                               ' in hexes ' +
                               str(mesh.hex_min_edge_length) + '\n')
        else:
            totstat_file.write('minimum edge length: ' +
                               str(mesh.min_edge_length) +
                               ' in ' + str(len(mesh.hex_min_edge_length)) +
                               ' hexes.' + '\n')
        if len(mesh.hex_max_edge_length) <= 30:
            totstat_file.write('maximum edge length: ' +
                               str(mesh.max_edge_length) +
                               ' in hexes ' +
                               str(mesh.hex_max_edge_length) + '\n')
        else:
            totstat_file.write('maximum edge length: ' +
                               str(mesh.max_edge_length) +
                               ' in ' + str(len(mesh.hex_max_edge_length)) +
                               ' hexes.' + '\n')
        totstat_file.write('-' * 70 + '\n')
        ...
        for i in range(0, mesh.nbin + 1):
            if i in mesh.edgemax_hystogram.keys():
                ...
                totstat_file.write(
                    str(i) + ' [' +
                    str(i * factor + mesh.min_edge_length) + '->' +
                    str((i + 1) * factor + mesh.min_edge_length) + '[ : 0/' +
                    str(tot) + ' hexes (0%)' + '\n')
    try:
