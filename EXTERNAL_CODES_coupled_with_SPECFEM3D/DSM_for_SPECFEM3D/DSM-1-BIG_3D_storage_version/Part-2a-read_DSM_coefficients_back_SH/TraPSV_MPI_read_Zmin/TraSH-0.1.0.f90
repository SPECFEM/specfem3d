...
!  call MPI_Bcast(nzone,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
 ...
 if (mybigrank == 0) then
  ...
  read(1,*) nsta_global
!!now the nsta_global is stations number read from file stationsinf.
!  r_n = nsta
!  theta_n = nsta
 endif
...
! it is sufficient for us to save the final result to disk in single precision
  ...
  allocate(tmp_tabg0der(1:maxlmax_g,1,6,-2:2))
...
!! now these array for storing the spherical harmonics vector is not required.
...
!!                               call calup
!                                u1 = dcmplx( 0.d0 )
!                                u2 = g0tmp(1)*dvec(itheta,2,m)*inv_lsq
!                                u3 = g0tmp(1)*dvec(itheta,3,m)*inv_lsq

!                                udr1 = dcmplx( 0.d0 )
!                                udr2 = g0dertmp(1)*dvec(itheta,2,m)*inv_lsq
!                                udr3 = g0dertmp(1)*dvec(itheta,3,m)*inv_lsq

!                                udt1 = dcmplx( 0.d0 )
!                                udt2 = g0tmp(1)*dvecdt(itheta,2,m)*inv_lsq
!                                udt3 = g0tmp(1)*dvecdt(itheta,3,m)*inv_lsq

!                                udp1 = dcmplx( 0.d0 )
!                                udp2 = g0tmp(1)*dvecdp(itheta,2,m)*inv_lsq
!                                udp3 = g0tmp(1)*dvecdp(itheta,3,m)*inv_lsq
!if (myrank ==0)  write(*,*) 'g0tmp and g0dertmp 1st element is:',g0tmp(1),g0dertmp(1)
...
!! call udertoStress
!  stress(1,1,itheta) = stress(1,1,itheta)+Cval*uder11+Fval*uder22+Fval*uder33
!  stress(2,1,itheta) = stress(2,1,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder33
!  stress(3,1,itheta) = stress(3,1,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder22

!  stress(4,1,itheta) = stress(4,1,itheta)+Lval*(uder12+uder21)
!  stress(5,1,itheta) = stress(5,1,itheta)+Lval*(uder13+uder31)
!  stress(6,1,itheta) = stress(6,1,itheta)+Nval*(uder23+uder32)
...
!                               call calup
                                u1 = dcmplx( 0.d0 )
                                u2 = g0tmp(2)*dvec(itheta,2,m)*inv_lsq
                                u3 = g0tmp(2)*dvec(itheta,3,m)*inv_lsq

                                udr1 = dcmplx( 0.d0 )
                                udr2 = g0dertmp(2)*dvec(itheta,2,m)*inv_lsq
                                udr3 = g0dertmp(2)*dvec(itheta,3,m)*inv_lsq

                                udt1 = dcmplx( 0.d0 )
                                udt2 = g0tmp(2)*dvecdt(itheta,2,m)*inv_lsq
                                udt3 = g0tmp(2)*dvecdt(itheta,3,m)*inv_lsq

                                udp1 = dcmplx( 0.d0 )
                                udp2 = g0tmp(2)*dvecdp(itheta,2,m)*inv_lsq
                                udp3 = g0tmp(2)*dvecdp(itheta,3,m)*inv_lsq
...
! call udertoStress
  stress(1,2,itheta) = stress(1,2,itheta)+Cval*uder11+Fval*uder22+Fval*uder33
  stress(2,2,itheta) = stress(2,2,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder33
  stress(3,2,itheta) = stress(3,2,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder22

  stress(4,2,itheta) = stress(4,2,itheta)+Lval*(uder12+uder21)
  stress(5,2,itheta) = stress(5,2,itheta)+Lval*(uder13+uder31)
  stress(6,2,itheta) = stress(6,2,itheta)+Nval*(uder23+uder32)
...
!                               call calup
                                u1 = dcmplx( 0.d0 )
                                u2 = g0tmp(3)*dvec(itheta,2,m)*inv_lsq
                                u3 = g0tmp(3)*dvec(itheta,3,m)*inv_lsq

                                udr1 = dcmplx( 0.d0 )
                                udr2 = g0dertmp(3)*dvec(itheta,2,m)*inv_lsq
                                udr3 = g0dertmp(3)*dvec(itheta,3,m)*inv_lsq

                                udt1 = dcmplx( 0.d0 )
                                udt2 = g0tmp(3)*dvecdt(itheta,2,m)*inv_lsq
                                udt3 = g0tmp(3)*dvecdt(itheta,3,m)*inv_lsq

                                udp1 = dcmplx( 0.d0 )
                                udp2 = g0tmp(3)*dvecdp(itheta,2,m)*inv_lsq
                                udp3 = g0tmp(3)*dvecdp(itheta,3,m)*inv_lsq
...
! call udertoStress
  stress(1,3,itheta) = stress(1,3,itheta)+Cval*uder11+Fval*uder22+Fval*uder33
  stress(2,3,itheta) = stress(2,3,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder33
  stress(3,3,itheta) = stress(3,3,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder22

  stress(4,3,itheta) = stress(4,3,itheta)+Lval*(uder12+uder21)
  stress(5,3,itheta) = stress(5,3,itheta)+Lval*(uder13+uder31)
  stress(6,3,itheta) = stress(6,3,itheta)+Nval*(uder23+uder32)

                                    displacement(1,3,itheta) = u1 + displacement(1,3,itheta)
                                    displacement(2,3,itheta) = u2 + displacement(2,3,itheta)
                                    displacement(3,3,itheta) = u3 + displacement(3,3,itheta)
...
!                               call calup
                                u1 = dcmplx( 0.d0 )
                                u2 = g0tmp(4)*dvec(itheta,2,m)*inv_lsq
                                u3 = g0tmp(4)*dvec(itheta,3,m)*inv_lsq

                                udr1 = dcmplx( 0.d0 )
                                udr2 = g0dertmp(4)*dvec(itheta,2,m)*inv_lsq
                                udr3 = g0dertmp(4)*dvec(itheta,3,m)*inv_lsq

                                udt1 = dcmplx( 0.d0 )
                                udt2 = g0tmp(4)*dvecdt(itheta,2,m)*inv_lsq
                                udt3 = g0tmp(4)*dvecdt(itheta,3,m)*inv_lsq

                                udp1 = dcmplx( 0.d0 )
                                udp2 = g0tmp(4)*dvecdp(itheta,2,m)*inv_lsq
                                udp3 = g0tmp(4)*dvecdp(itheta,3,m)*inv_lsq
...
! call udertoStress
  stress(1,4,itheta) = stress(1,4,itheta)+Cval*uder11+Fval*uder22+Fval*uder33
  stress(2,4,itheta) = stress(2,4,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder33
  stress(3,4,itheta) = stress(3,4,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder22

  stress(4,4,itheta) = stress(4,4,itheta)+Lval*(uder12+uder21)
  stress(5,4,itheta) = stress(5,4,itheta)+Lval*(uder13+uder31)
  stress(6,4,itheta) = stress(6,4,itheta)+Nval*(uder23+uder32)
...
!                               call calup
                                u1 = dcmplx( 0.d0 )
                                u2 = g0tmp(5)*dvec(itheta,2,m)*inv_lsq
                                u3 = g0tmp(5)*dvec(itheta,3,m)*inv_lsq

                                udr1 = dcmplx( 0.d0 )
                                udr2 = g0dertmp(5)*dvec(itheta,2,m)*inv_lsq
                                udr3 = g0dertmp(5)*dvec(itheta,3,m)*inv_lsq

                                udt1 = dcmplx( 0.d0 )
                                udt2 = g0tmp(5)*dvecdt(itheta,2,m)*inv_lsq
                                udt3 = g0tmp(5)*dvecdt(itheta,3,m)*inv_lsq

                                udp1 = dcmplx( 0.d0 )
                                udp2 = g0tmp(5)*dvecdp(itheta,2,m)*inv_lsq
                                udp3 = g0tmp(5)*dvecdp(itheta,3,m)*inv_lsq
...
! call udertoStress
  stress(1,5,itheta) = stress(1,5,itheta)+Cval*uder11+Fval*uder22+Fval*uder33
  stress(2,5,itheta) = stress(2,5,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder33
  stress(3,5,itheta) = stress(3,5,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder22

  stress(4,5,itheta) = stress(4,5,itheta)+Lval*(uder12+uder21)
  stress(5,5,itheta) = stress(5,5,itheta)+Lval*(uder13+uder31)
  stress(6,5,itheta) = stress(6,5,itheta)+Nval*(uder23+uder32)
...
!                               call calup
                                u1 = dcmplx( 0.d0 )
                                u2 = g0tmp(6)*dvec(itheta,2,m)*inv_lsq
                                u3 = g0tmp(6)*dvec(itheta,3,m)*inv_lsq

                                udr1 = dcmplx( 0.d0 )
                                udr2 = g0dertmp(6)*dvec(itheta,2,m)*inv_lsq
                                udr3 = g0dertmp(6)*dvec(itheta,3,m)*inv_lsq

                                udt1 = dcmplx( 0.d0 )
                                udt2 = g0tmp(6)*dvecdt(itheta,2,m)*inv_lsq
                                udt3 = g0tmp(6)*dvecdt(itheta,3,m)*inv_lsq

                                udp1 = dcmplx( 0.d0 )
                                udp2 = g0tmp(6)*dvecdp(itheta,2,m)*inv_lsq
                                udp3 = g0tmp(6)*dvecdp(itheta,3,m)*inv_lsq
...
! call udertoStress
  stress(1,6,itheta) = stress(1,6,itheta)+Cval*uder11+Fval*uder22+Fval*uder33
  stress(2,6,itheta) = stress(2,6,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder33
  stress(3,6,itheta) = stress(3,6,itheta)+Fval*uder11+Aval*uder22+Aval*uder33-2.d0*Nval*uder22

  stress(4,6,itheta) = stress(4,6,itheta)+Lval*(uder12+uder21)
  stress(5,6,itheta) = stress(5,6,itheta)+Lval*(uder13+uder31)
  stress(6,6,itheta) = stress(6,6,itheta)+Nval*(uder23+uder32)

                                    displacement(1,6,itheta) = u1 + displacement(1,6,itheta)
...
!        stresssngl(1:6,1:6,1:theta_n) = stress(1:6,1:6,1:theta_n)
    ...
    stresssngl(3,1,ista) = stress(3,1,ista) * inverse_of_omega_complex
    stresssngl(4,1,ista) = stress(4,1,ista) * inverse_of_omega_complex
    stresssngl(5,1,ista) = stress(5,1,ista) * inverse_of_omega_complex
    stresssngl(6,1,ista) = stress(6,1,ista) * inverse_of_omega_complex
    stresssngl(1,2,ista) = stress(1,2,ista) * inverse_of_omega_complex
    stresssngl(2,2,ista) = stress(2,2,ista) * inverse_of_omega_complex
    stresssngl(3,2,ista) = stress(3,2,ista) * inverse_of_omega_complex
    stresssngl(4,2,ista) = stress(4,2,ista) * inverse_of_omega_complex
    stresssngl(5,2,ista) = stress(5,2,ista) * inverse_of_omega_complex
    stresssngl(6,2,ista) = stress(6,2,ista) * inverse_of_omega_complex
    stresssngl(1,3,ista) = stress(1,3,ista) * inverse_of_omega_complex
    stresssngl(2,3,ista) = stress(2,3,ista) * inverse_of_omega_complex
    stresssngl(3,3,ista) = stress(3,3,ista) * inverse_of_omega_complex
    stresssngl(4,3,ista) = stress(4,3,ista) * inverse_of_omega_complex
    stresssngl(5,3,ista) = stress(5,3,ista) * inverse_of_omega_complex
    stresssngl(6,3,ista) = stress(6,3,ista) * inverse_of_omega_complex
    stresssngl(1,4,ista) = stress(1,4,ista) * inverse_of_omega_complex
    stresssngl(2,4,ista) = stress(2,4,ista) * inverse_of_omega_complex
    stresssngl(3,4,ista) = stress(3,4,ista) * inverse_of_omega_complex
    stresssngl(4,4,ista) = stress(4,4,ista) * inverse_of_omega_complex
    stresssngl(5,4,ista) = stress(5,4,ista) * inverse_of_omega_complex
    stresssngl(6,4,ista) = stress(6,4,ista) * inverse_of_omega_complex
    stresssngl(1,5,ista) = stress(1,5,ista) * inverse_of_omega_complex
    stresssngl(2,5,ista) = stress(2,5,ista) * inverse_of_omega_complex
    stresssngl(3,5,ista) = stress(3,5,ista) * inverse_of_omega_complex
    stresssngl(4,5,ista) = stress(4,5,ista) * inverse_of_omega_complex
    stresssngl(5,5,ista) = stress(5,5,ista) * inverse_of_omega_complex
    stresssngl(6,5,ista) = stress(6,5,ista) * inverse_of_omega_complex
    stresssngl(1,6,ista) = stress(1,6,ista) * inverse_of_omega_complex
    stresssngl(2,6,ista) = stress(2,6,ista) * inverse_of_omega_complex
    stresssngl(3,6,ista) = stress(3,6,ista) * inverse_of_omega_complex
...
!        displacementsngl(1:3,1:6,1:theta_n) = displacement(1:3,1:6,1:theta_n)
    ...
    displacementsngl(3,1,ista) = displacement(3,1,ista)
    displacementsngl(1,2,ista) = displacement(1,2,ista)
    displacementsngl(2,2,ista) = displacement(2,2,ista)
    displacementsngl(3,2,ista) = displacement(3,2,ista)
    displacementsngl(1,3,ista) = displacement(1,3,ista)
    displacementsngl(2,3,ista) = displacement(2,3,ista)
    displacementsngl(3,3,ista) = displacement(3,3,ista)
    displacementsngl(1,4,ista) = displacement(1,4,ista)
    displacementsngl(2,4,ista) = displacement(2,4,ista)
    displacementsngl(3,4,ista) = displacement(3,4,ista)
    displacementsngl(1,5,ista) = displacement(1,5,ista)
    displacementsngl(2,5,ista) = displacement(2,5,ista)
    displacementsngl(3,5,ista) = displacement(3,5,ista)
    displacementsngl(1,6,ista) = displacement(1,6,ista)
...
!!write(*,*) ir0, 'coutfile and coutfile1 is:',coutfile,coutfile1
...
!       close(35)
        ...
        write(1,rec=i+1) displacementsngl_global(1:3,1:6,1:nsta_global)
        close(1)
