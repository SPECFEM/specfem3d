subroutine pinputTra(outputDir,psvmodel,modelname,stationsinf,tlen,imin,imax,r0min, &
  ...
  implicit none
  character(len=250) tmpfile
  character(len=250) :: dummy,outputDir,psvmodel,modelname,stationsinf,inputdir,inputIASP
...
!!  3 : pp : YY (?signe + vers le Nord?) (c'est apres la rotation que l'on choisi le sens)
!!  4 : rt : ZX
!!  5 : rp : ZY
!!  6 : tp : XY
!!
...
!!
!!
subroutine Local2chunk(vec,n,np1,Origin_Lat_Chunk,Orign_Lon_Chunk,Lat_current,lon_current,normal_ref,imin,imax,bazi,azi_chunk)
  implicit none
  integer n,np1
  complex(kind(0d0)) vec(n,0:2*np1-1)
  real(kind(0d0)) Bazi,azi_chunk,Bazi_chunk,Orign_Lon_Chunk,Origin_Lat_Chunk,Lat_current,Lon_current
  ! normales
  complex(kind(0d0)) normal_RT(3),normal_local(3),normal_ref(3),normal(3),normal_global(3)
  ! tractions
  complex(kind(0d0)) Trac_local(3),Trac_RT(3),Trac_chunk(3),Trac_global(3)
  ! deplacement
  complex(kind(0d0)) Disp_global(3),Disp_local(3),Disp_RT(3),Disp_chunk(3)
  ! sorties dsm repere ZRT
  complex(kind(0d0)) Stress(3,3),Disp(3)
  ! matrices de passage repere ZEN
  complex(kind(0d0)) Pass_local_chunk(3,3),Pass_chunk_local(3,3)
  ! matrice de passaies repere ZRT <-> ZEN
  complex(kind(0d0)) Rot_azi_chunk(3,3),Rot_RT_NE(3,3),Pass_Chunk_global(3,3),Pass_global_chunk(3,3)
  ! matice passage XYZ <-> ZEN
  complex(kind(0d0)) Pass_local_global(3,3),Pass_global_local(3,3)
  ! matrice de passages repere global XYZ
  complex(kind(0d0)) RL2L(3,3),RL2Li(3,3),Razi(3,3),Razi_inv(3,3)
  integer i,j,k
  integer icomp,itime,imin,imax

  ! matirce de rotation (ZRT (dsm) -> ZEN)
  call RotationMatrix_RT2Local(Rot_RT_NE,Bazi) !! c'est le bazi de la source
  Bazi_chunk=270.d0-azi_chunk ! car on fait cette operation dans le subroutine
  ...
  normal(:)=normal_ref(:)
  call permute2dsm(normal)  ! on passe dans le systeme ZEN
  !write(100,*) 'Pass_chunk_global'
  !call Display_matrix_realpart(100,Pass_Chunk_global)
  !write(100,*) 'Passs_local_gobal'
  !call Display_matrix_realpart(100,Pass_local_global)
  !write(100,*)
  !write(100,*) 'normale chnuk', normal_ref
  !write(100,*)
  ! je veux exprimer la normale dans le repere local du point geographique considere
  ! normal=Razi_inv*normal je met le normale dans le repere Est-Nord au centre du chunk
  ! c'est le repere local du centre du Chunk
  !write(100,*) 'normale ZEN', normal
  !write(100,*)

  ! on tourne de l'azimuth du chunk en sens  inverse
  call MultInv(Rot_azi_chunk,normal,normal_global)
  ! je passse du chunk au repere local
  call MultDir(Pass_chunk_local,normal_global,normal_local)
  !write(100,*) ' Matice chunk_local :'

  !write(100,*)
  !write(100,*) 'normale locale', normal_local
  !write(100,*)
  ! on se met dans le repere du Stress
  call MultInv(Rot_RT_NE,normal_local,normal_RT) ! on est dans le repere RT
  !write(100,*) 'normale RT', normal_RT
  !write(100,*)

  !write(100,*)'-------------------------------------------------------------------------------------------------------------------'
     ! TO DO : il faut se mettre dans le repere du Stress
     do itime=imin,imax !0,2*np1-1
       ...
       ! on calcule la traction dans le repere RT
       call MultDir(Stress,normal_RT,Trac_RT)
       ! on se remet dans le repere local
       call MultDir(Rot_RT_NE,Trac_RT,Trac_local)
       ! on passe dans le repere local du chunk
...
subroutine permute2dsm(x)
  complex(kind(0d0)) x(3),a,b,c
  a=x(3)  ! bas  - > haut
  b=x(2)  ! Sud  - > Nord
  c=x(1)  ! West - > Est
...
end subroutine permute2dsm

subroutine permute2global(x)
  complex(kind(0d0)) x(3),a,b,c
  a=x(1)  ! bas  - > haut
  b=x(3)  ! Sud  - > Nord
  c=x(2)  ! West - > Est
...
end subroutine permute2global



subroutine Local2chunkold(vec,n,np1,Orign_Lon_Chunk,Origin_Lat_Chunk,Lat_current,Lon_current,normal,Bazi,imin,imax)
  implicit none
  integer n,np1
  complex(kind(0d0)) vec(n,0:2*np1-1)
  real(kind(0d0)) Bazi,Orign_Lon_Chunk,Origin_Lat_Chunk,Lat_current,Lon_current!,normal(3)
  complex(kind(0d0)) normal_RT(3),normal_local(3),Pass_local_chunk(3,3),Pass_chunk_local(3,3),normal(3)
  complex(kind(0d0)) Rot_RT_NE(3,3),Pass_Chunk_global(3,3),Pass_global_chunk(3,3),Pass_tmp(3,3)
  complex(kind(0d0)) Pass_local_global(3,3),Pass_global_local(3,3),Stress(3,3),Disp(3)
  complex(kind(0d0)) Trac_local(3),Trac_RT(3),Trac_chunk(3),Disp_local(3),Disp_chunk(3)
  integer i,j,k
  integer icomp,itime,imin,imax
  ...
  !normal(3)=dcmplx(0.d0)
  call MultDir(Pass_chunk_local,normal,normal_local)
  !if (itime>1000 .and. itime < 1002) then
  !write(100,*) '----------NORMALES---------------------',Lon_current,Lat_current
  !write(100,*) 'normal donnee' ,normal
  !write(100,*)
  !write(100,*) 'normal local  ',  normal_local
  !write(100,*)
  !endif
  ...
  !write(100,*)
  !call  MatrixMultInverse(Pass_chunk_global,Pass_global_chunk,Pass_tmp)
  !write(100,*) Pass_tmp
  !write(100,*)
  ...
  !write(100,*) Pass_chunk_local
     ...
     !if (itime>49 .and. itime < 51) then
     !*write(*,*) 'vec ',vec(:,itime)
     !*write(*,*) 'Stress ',Stress
     !write(100,*) vec(7,itime),vec(8,itime),vec(9,itime)
     !write(*,*) '------------------------'
     !endif
      !!!!!!!!!!!!!!!!!   A FAIRE !!!!!!!!!!!!!!!!
      ...
      !if (itime>1000 .and. itime < 1002)  write(100,*) 'normal RT',normal_RT
      call MultDir(Stress,normal_RT,Trac_RT)
      !if (itime>100 .and. itime < 102)  write(100,*) 'Trac RT ',Trac_RT
      !! ROTATION DU REPRE RT -> CHUNK
      ! Trac_local = Rot_RT_NE * Trac_RT
      call MultDir(Rot_RT_NE,Trac_RT,Trac_local)
      !if (itime>100 .and. itime < 102)  write(100,*) 'Trac local ',Trac_local
      ! Trac_chunk = Rot_local_chunk * Trac_local
      call MultDir(Pass_local_chunk,Trac_local,Trac_chunk)
      !if (itime>100 .and. itime < 102)  write(100,*) 'Trac chunk ',Trac_chunk
      ...
      ! traction (1,2,3) (X,Y,Z chunk)
       ...
       !!  ROTATION DU REPRE RT -> CHUNK
       call MultDir(Rot_RT_NE,Disp,Disp_local)
       call MultDir(Pass_local_chunk,Disp_local,Disp_chunk)
...
subroutine RotationMatrix(lat,lon,R)
 ...
 ! vm changement de signe (pour pointer vers le nord)
 R(1,3)= - ct*cp
 R(2,3)= - ct*sp
 R(3,3)=  st
...
subroutine InputForTraction(Orign_Lon_Chunk,Origin_Lat_Chunk,azi_chunk,normal,delta_lat,delta_lon,face)
 implicit none
 real(kind(0.d0)) Orign_Lon_Chunk,Origin_Lat_Chunk,normal_real(3)
 real(kind(0.d0)) azi_chunk,delta_lat,delta_lon,deg2rad
 ...
 ! normale dans le chunk
 normal(:)=dcmplx(0.d0)
 select case (face)
   case('xmin')
     normal(1)=dcmplx(-dcos(0.5d0*delta_lon*deg2rad))
     normal(2)=dcmplx(0.d0)
     normal(3)=dcmplx(-dsin(0.5d0*delta_lon*deg2rad))
   case('xmax')
     normal(1)=dcmplx(dcos(0.5d0*delta_lon*deg2rad))
     normal(2)=dcmplx(0.d0)
     normal(3)=dcmplx(-dsin(0.5d0*delta_lon*deg2rad))
   case('ymin')
     normal(1)=dcmplx(0.d0)
     normal(2)=dcmplx(-dcos(0.5d0*delta_lat*deg2rad))
     normal(3)=dcmplx(-dsin(0.5d0*delta_lat*deg2rad))
    case('ymax')
     normal(1)=dcmplx(0.d0)
     normal(2)=dcmplx(dcos(0.5d0*delta_lat*deg2rad))
     normal(3)=dcmplx(-dsin(0.5d0*delta_lat*deg2rad))
    case('zmin') ! c'est faux pour le momment, il faut revoir pour zmin (ca change en fonction du point)
     normal(1)=dcmplx(0.d0)
     normal(2)=dcmplx(0.d0)
     normal(3)=dcmplx(-1.d0)
 end select
 !write(100,*) 'FACE ',face
 !write(100,*) 'NORMALE ', normal
