...
\end{description}
There are quite a few more parameters to change the default setup of your runs. Please check the comments in the \texttt{Par\_file} directly for further explanations.\\


The present version of SPECFEM can handle fully saturated porous simulations.
(Christina Morency implemented Biot's equation.
However, the code cannot calculate partially saturated cases in its state.
...
We use a file \texttt{nummaterial\_poroelastic\_file}, which is located in the directory \texttt{MESH/}. The line format is as follow:
