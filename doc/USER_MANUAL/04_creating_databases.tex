...
\item [{\texttt{USE\_OTHER\_TIME\_FUNCTION}}] Turn this flag on to use other source time functions for the CMTSOLUTION (Brune and Smooth Brune). If this flag is on, one line with the other source time function name (\textt{brune} or \texttt{smooth\_brune}) must be added at the end of each \texttt{CMTSOLUTION}.
