...
for (( iproc = 0; iproc < $NPROC; iproc++ )); do
  ...
  if [ -e OUTPUT_FILES.syn.forward/${rank}_dx_SU ]; then
    syn=OUTPUT_FILES.syn.forward/${rank}_dx_SU
    dat=OUTPUT_FILES.dat.forward/${rank}_dx_SU
  ...
  if [ -e OUTPUT_FILES.syn.forward/${rank}_dy_SU ]; then
    syn=OUTPUT_FILES.syn.forward/${rank}_dy_SU
    dat=OUTPUT_FILES.dat.forward/${rank}_dy_SU
    echo "> ./adj_seismogram.py $syn $dat"
    echo
    # adjoint source f^adj = (s - d)
    ./adj_seismogram.py $syn $dat
  ...
  if [ -e OUTPUT_FILES.syn.forward/${rank}_dz_SU ]; then
    syn=OUTPUT_FILES.syn.forward/${rank}_dz_SU
    dat=OUTPUT_FILES.dat.forward/${rank}_dz_SU
    echo "> ./adj_seismogram.py $syn $dat"
    echo
    # adjoint source f^adj = (s - d)
    ./adj_seismogram.py $syn $dat
...
# takes a small fraction of the absolute perturbation for model update scaling length
update_percent=`echo "$update_percent" | awk '{print $0 * 0.1}'`
