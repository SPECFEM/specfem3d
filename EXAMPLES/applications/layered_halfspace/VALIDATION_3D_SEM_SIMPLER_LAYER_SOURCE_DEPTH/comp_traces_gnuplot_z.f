...
c DK DK bouchon uses another definition
...
c--- sauvegarde data gnuplot
 ...
 70   format('datazbouchon',i3.3,'.gnu')
      open(unit=17,file=namefile,status='unknown')
      do it=1,ntbouchon
...
c petit script de visu pour gnuplot
      open(unit=17,file='plotall_z.gnu',status='unknown')
	write(17,*) 'set term x11'
      write(17,*) 'set title "Vertical component"'
      write(17,*) 'set xrange [0:40]'
      do irec=nrec/2+1,nrec
 ...
 100	format('plot "datazspec',i3.3,'.gnu" w l 1, "datazbouchon',i3.3
     .		,'.gnu" w l 2')
