...
foreach isrc (${num_src_arr})
    foreach isim  (${srcapp})
        ...
        echo "#PBS -S /bin/bash" > submit_$outputname.sh
        echo "#PBS -N $1" >> submit_$outputname.sh
        echo "#PBS -e $1.err" >> submit_$outputname.sh
        echo "#PBS -o $1.out">> submit_$outputname.sh
        echo "#PBS -l select=4:ncpus=8:mpiprocs=8">> submit_$outputname.sh
        echo "#PBS -l walltime=2:00:00">> submit_$outputname.sh
        echo 'cat $PBS_NODEFILE'>> submit_$outputname.sh
        echo 'cd $PBS_O_WORKDIR'>> submit_$outputname.sh
        echo ". /etc/profile.d/modules.sh">> submit_$outputname.sh
        echo "module purge">> submit_$outputname.sh
        echo "module load intel/12.1.3">> submit_$outputname.sh
        echo "module load intelmpi/4.0.3">> submit_$outputname.sh
#       echo "module load CINES/20110112">> submit_$outputname.sh
#       echo "module load openmpi/1.4.4" >> submit_$outputname.sh
        echo 'cat $PBS_NODEFILE | uniq > mpd.hosts'>> submit_$outputname.sh
        echo 'mpdboot --rsh=ssh -v -n `cat mpd.hosts|wc -l` -f mpd.hosts'>> submit_$outputname.sh
        echo 'nprocs=$(cat $PBS_NODEFILE|wc -l)'>> submit_$outputname.sh
#        echo "ulimit -s unlimited">> submit_$outputname.sh
#       echo 'TMPDIR="./"' >> submit_$outputname.sh
        echo 'mpiexec -np $nprocs $PBS_O_WORKDIR/axisem'>> submit_$outputname.sh
        echo "mpdallexit">> submit_$outputname.sh




	echo "submit_$outputname.sh" 
        chmod +x submit_$outputname.sh
        qsub submit_$outputname.sh
