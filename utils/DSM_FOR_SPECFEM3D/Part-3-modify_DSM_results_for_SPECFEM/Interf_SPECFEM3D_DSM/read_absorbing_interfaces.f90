...
!
       ...
       do iface = 1,num_abs_boundary_faces ! boucle sur les facettes
          ...
          if (iref.eq.1) code_face = 1
          if (iref.eq.NGLLX) code_face = 2
          if (jref.eq.1) code_face = 3
          if (jref.eq.NGLLY) code_face = 4
          if (kref.eq.1) code_face=5

          ispec2d = IndSpec_face(ispec_global,code_face)

          ! xmin 
          if (code_face.eq.1) then
             do igll=1,NGLLSQUARE
               ...
               if(ibloc.ge.iblocmin) write(28) TracXmin(:,:,j,k,ispec2d)
               if(ibloc.ge.iblocmin) write(29) VelXmin(:,:,j,k,ispec2d)    
             end do
          end if


          ! xmax 
          if (code_face.eq.2) then
             do igll=1,NGLLSQUARE
               i = abs_boundary_ijk(1,igll,iface)
               j = abs_boundary_ijk(2,igll,iface)
               k = abs_boundary_ijk(3,igll,iface)
               ...
               if(ibloc.ge.iblocmin) write(28) TracXmax(:,:,j,k,ispec2d)
               if(ibloc.ge.iblocmin) write(29) VelXmax(:,:,j,k,ispec2d)
            end do
         end if


         ! ymin 
         if (code_face.eq.3) then
            do igll=1,NGLLSQUARE
               i = abs_boundary_ijk(1,igll,iface)
               j = abs_boundary_ijk(2,igll,iface)
               k = abs_boundary_ijk(3,igll,iface)
               ...
               !write(27,'(3f20.5,10x,3f20.5)') x0,y0,z0,x1,y1,z1
               if(ibloc.ge.iblocmin) write(28) TracYmin(:,:,i,k,ispec2d)
               if(ibloc.ge.iblocmin) write(29) VelYmin(:,:,i,k,ispec2d)
         ...
         if (code_face.eq.4) then
           do igll=1,NGLLSQUARE
              ...
              !write(27,'(3f20.5,10x,3f20.5)') x0,y0,z0,x1,y1,z1
              if(ibloc.ge.iblocmin) write(28) TracYmax(:,:,i,k,ispec2d)
              if(ibloc.ge.iblocmin) write(29) VelYmax(:,:,i,k,ispec2d)
         ...
         if (code_face.eq.5) then
            do igll=1,NGLLSQUARE
              ...
              !write(27,'(3f20.5,10x,3f20.5)') x0,y0,z0,x1,y1,z1
              if(ibloc.ge.iblocmin) write(28) TracZmin(:,:,i,j,ispec2d)
              if(ibloc.ge.iblocmin) write(29) VelZmin(:,:,i,j,ispec2d)
            end do
         end if
