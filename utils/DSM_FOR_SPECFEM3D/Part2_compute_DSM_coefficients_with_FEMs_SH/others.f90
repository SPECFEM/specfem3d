subroutine pinputTra_Part1(outputDir,psvmodel,modelname,stationsinf,tlen,imin,imax,r0min,r0max,r0delta,r0lat,r0lon,itranslat,myrank)
 implicit none
...
! write(*,*) 'outputDir,psvmodel,modelname,stationsinf is :'
! write(*,*) outputDir,psvmodel,modelname,stationsinf
 read(1,*) tlen
 ...
 read(1,*) imin,imax
 read(1,*) itranslat
! write(*,*) 'tlen,r0min,r0lat,r0lon is:'
! write(*,*) tlen,r0min,r0lat,r0lon
! write(*,*) 'imin,imax,itranslat is:'
! write(*,*) imin,imax,itranslat
...
subroutine calra2(nlayer,gridpar,dzpar,nzone,vrmin,vrmax,rmin,rmax,nnl,ra,re,nsta,rsta,rrsta, iista,updown)
  ...
  integer :: izone,itmp,i,ii,nsta,ista, iista(1:3,1:nsta), ciista,updown(1:nsta)
...
!	        write(*,*) '!!!!!!special radius',rsta(ista),ista,updown(ista)
...
subroutine calamp_vector(g,l,lsuf,maxamp,ismall,ratl,r_n)
  
  implicit none
  integer :: l,lsuf,r_n,ir_
  integer :: ismall(1:r_n)
  real(kind(0d0)) :: maxamp(1:r_n),ratl
  complex(kind(0d0)) :: g
  real(kind(0d0)) :: amp,ampratio    
 do  ir_ = 1,r_n
  ampratio = 0.d0
  amp = abs(g)
  if( amp.gt.maxamp(ir_) ) maxamp(ir_) = amp
  if ( (amp /= 0.d0).and.(maxamp(ir_) /= 0.d0) ) then
     ampratio = amp / maxamp(ir_)
  endif
  if( (ampratio.lt.ratl).and.(l.gt.lsuf) ) then
     ismall(ir_) = ismall(ir_) + 1
  else
     ismall(ir_) = 0
  endif
 enddo
  
end subroutine calamp_vector
